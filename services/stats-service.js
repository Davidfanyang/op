...
class StatsService {
  ...
  async getTrainingStats(filters = {}) {
    ...
    if (endTime) { conditions.push('ts.started_at <= ?'); params.push(endTime); }
    ...
    const totalSql = `
      SELECT 
        ...
        SUM(CASE WHEN ts.status = 'finished' THEN 1 ELSE 0 END) as finished_sessions,
        SUM(CASE WHEN ts.status IN ('cancelled', 'interrupted') THEN 1 ELSE 0 END) as interrupted_sessions,
        AVG(ts.total_rounds) as avg_rounds
    ...
    const scenarioSql = `
      SELECT ts.scenario_id, ts.scenario_title, COUNT(*) as count
      FROM training_sessions ts ${whereClause}
      GROUP BY ts.scenario_id, ts.scenario_title ORDER BY count DESC
    `;
    const scenarioDistribution = await this.pool.queryMany(scenarioSql, params);

    const agentSql = `
      SELECT ts.agent_id, COUNT(*) as session_count, COALESCE(SUM(ts.total_rounds), 0) as round_count
      FROM training_sessions ts ${whereClause}
      GROUP BY ts.agent_id ORDER BY session_count DESC
    `;
    ...
    return {
      ...
      data: {
        ...
        avg_rounds: totalStats.avg_rounds ? parseFloat(Number(totalStats.avg_rounds).toFixed(2)) : 0,
  ...
  async getQualityStats(filters = {}) {
    ...
    if (startTime) { sessionConditions.push('ls.started_at >= ?'); sessionParams.push(startTime); }
    if (endTime) { sessionConditions.push('ls.started_at <= ?'); sessionParams.push(endTime); }

    const sessionWhere = sessionConditions.length > 0 ? `WHERE ${sessionConditions.join(' AND ')}` : '';

    const sessionsSql = `SELECT COUNT(*) as total_sessions FROM live_sessions ls ${sessionWhere}`;
  ...
  async getAlertStats(filters = {}) {
    ...
    const typeSql = `SELECT a.alert_type, COUNT(*) as count FROM alerts a ${joinClause} ${whereClause} GROUP BY a.alert_type ORDER BY count DESC`;
  ...
  async getKnowledgeStats(filters = {}) {
    ...
    if (startTime) { conditions.push('kb.created_at >= ?'); params.push(startTime); }
    if (endTime) { conditions.push('kb.created_at <= ?'); params.push(endTime); }
    ...
    const totalSql = `
      SELECT 
        ...
        SUM(CASE WHEN kb.status = 'active' THEN 1 ELSE 0 END) as active_count,
        SUM(CASE WHEN kb.status = 'deprecated' THEN 1 ELSE 0 END) as deprecated_count
      FROM knowledge_base kb ${whereClause}
    `;
    const totalStats = await this.pool.queryOne(totalSql, params);

    const scenarioSql = `SELECT kb.scenario, COUNT(*) as count FROM knowledge_base kb ${whereClause} GROUP BY kb.scenario ORDER BY count DESC`;
    const scenarioDistribution = await this.pool.queryMany(scenarioSql, params);

    const sourceSql = `
      SELECT CASE WHEN kb.source_review_id IS NOT NULL THEN 'review' ELSE 'manual' END as source, COUNT(*) as count
      FROM knowledge_base kb ${whereClause} GROUP BY source ORDER BY count DESC
    `;
  ...
  async getTrendStats(filters = {}) {
    ...
    if (targetType === 'all' || targetType === 'quality') {
      ...
      if (endTime) { conditions.push('le.created_at <= ?'); params.push(endTime); }
      const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';

      const sql = `
        SELECT DATE(le.created_at) as date,
          COUNT(DISTINCT le.session_id) as quality_sessions,
          COUNT(*) as evaluations,
          SUM(CASE WHEN JSON_EXTRACT(le.output_payload, '$.problemType') = 'unknown' THEN 1 ELSE 0 END) as unknown_count
        FROM live_evaluations le ${whereClause}
  ...
  async getAgentStats(filters = {}) {
    ...
    const trainingSql = `
      SELECT ts.agent_id, COUNT(*) as training_sessions, COALESCE(SUM(ts.total_rounds), 0) as training_rounds
      FROM training_sessions ts ${whereClause} GROUP BY ts.agent_id
    `;
  ...
  async _aggregateQualityOverview(project, startTime, endTime) {
    ...
    const sql = `
      SELECT 
        ...
        SUM(CASE WHEN JSON_EXTRACT(le.output_payload, '$.problemType') = 'known' THEN 1 ELSE 0 END) as known_count,
        SUM(CASE WHEN JSON_EXTRACT(le.output_payload, '$.problemType') = 'unknown' THEN 1 ELSE 0 END) as unknown_count
      FROM live_evaluations le ${whereClause}
    `;
    const result = await this.pool.queryOne(sql, params);
  ...
  async _aggregateAlertOverview(project, startTime, endTime) {
    ...
    const sql = `
      SELECT 
        ...
        SUM(CASE WHEN a.alert_level = 'high' THEN 1 ELSE 0 END) as high_alert_count,
        SUM(CASE WHEN a.alert_level = 'medium' THEN 1 ELSE 0 END) as medium_alert_count
      FROM alerts a ${whereClause}
    `;
    const result = await this.pool.queryOne(sql, params);
